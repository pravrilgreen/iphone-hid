...
from ihc.rigs import GADGET_RIG_ID, ChipFound, GadgetFound, VideoFound, find_rigs, pair_gadget
...
def gadget_found():
    return GadgetFound("ihc", "fc000000.usb", {fn: f"/dev/hidg{i}" for i, fn in enumerate(g.ORDER)})


def test_gadget_takes_the_hdmi_input():
    spec = pair_gadget(gadget_found(), [CARD, HDMI])
    assert spec.id == GADGET_RIG_ID and spec.video == HDMI and spec.chip is None
...
def test_find_rigs_never_gives_the_hdmi_input_to_a_chip(monkeypatch):
    ...
    assert [(r.id, r.video.device) for r in rigs] == [("iphone", "/dev/video0"), ("rig-1-1.2", "/dev/video2")]
...
def test_auto_builds_the_box_rig(tmp_path, monkeypatch):
    ...
    try:
        (dev,) = reg.devices()
        assert dev.id == "iphone" and dev.hid is made[0] and opened == ["/dev/video0"]
        assert dev.calibration_path == tmp_path / "iphone.json"
        h = dev.check()
