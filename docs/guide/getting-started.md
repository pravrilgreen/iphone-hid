...
- `--model <model>`: chọn model mô phỏng (mặc định `iphone-15`; xem [simulator.md](../dev/simulator.md)).

Công cụ dòng lệnh cho chip HID giả:

```bash
ihc-hidtest --fake        # gõ `help`
```
...
[orange-pi-box.md](orange-pi-box.md).

1. Cài đặt iPhone theo [iphone-setup.md](iphone-setup.md).
2. Nối phần cứng theo [orange-pi-box.md](orange-pi-box.md#wiring): sạc 30 W trở lên vào cổng PD của hub, HDMI của hub vào
...
3. Trên board (image Ubuntu hoặc Debian của Orange Pi), trong thư mục repo đã cài ở mục 1 (venv đang bật), kiểm
   ...
   Không in gì thì xem mục Troubleshooting của [orange-pi-box.md](orange-pi-box.md#troubleshooting), hoặc dùng phương án dự
   phòng ở mục 5.
4. Cài hộp: gadget được dựng lúc boot (`ihc-gadget.service`), rồi server chạy `ihc serve --auto` (`ihc.service`).
   ...
   Từ mã nguồn thì dùng `sudo sh packaging/install.sh` (cài bằng pip). Cài bằng file release thì các lệnh là
   `ihc`, `ihc-hidtest`, `ihc-capture-check` (thay cho `ihc-hidtest`, `ihc-capture-check`).

5. Kiểm tra (mở khoá iPhone, chọn **Allow** nếu iOS hỏi về phụ kiện):

   ```bash
   ihc gadget status                                  # "USB state configured": iPhone đã nhận gadget
   ihc-capture-check list                # HDMI IN (rk_hdmirx hoặc snps_hdmirx)
   ```
   ...
   và đăng nhập lại). Các bài B2–B4 của [phase0-checklist.md](../research/phase0-checklist.md) làm chi tiết:
...
nhiều hộp dùng chung một token: `sudo IHC_TOKEN=<token> sh packaging/install.sh`. Từ máy khác trong mạng,
`IHC_TOKEN=<token> ihc devices` (hoặc `ihc devices --token-file <file>`) liệt kê mọi hộp và iPhone tìm được.

Muốn cố định cấu hình thay cho `--auto`, viết `farm.toml` theo mẫu trong [orange-pi-box.md](orange-pi-box.md) (`hid = { gadget =
...
1. Kiểm tra chip HID:
   ...
   ihc-hidtest --port <cổng> info
   ```
2. Kiểm tra nguồn hình:

   ```bash
   ihc-capture-check list
   ```
...
`sudo sh packaging/install.sh` cũng dùng được trên host không có chế độ device: khi đó nó không bật
`ihc-gadget.service`, chỉ chạy server.

Chi tiết từng bài kiểm tra ở [phase0-checklist.md](../research/phase0-checklist.md).
