...
4. Install the software. Download `ihc-box-<version>-linux-aarch64.run` from the
   ...
   From a source checkout instead: `sudo sh packaging/install.sh` (installs the same services with pip).
...
ihc-hidtest --gadget "info; type hello"
```
...
```
ihc-capture-check list          # the hdmirx node, and what the source sends
ihc-capture-check probe --device /dev/video0 --seconds 10
ihc-capture-check snapshot --device /dev/video0
```
...
| Control stops after the iPhone has been idle for a while | Auto-Lock put it to sleep: a sleeping iPhone suspends the USB bus and takes no input. Set Auto-Lock to Never and keep Low Power Mode off (see iphone-setup.md). To wake one that went to sleep: `ihc gadget wake`, then a key (`ihc-hidtest --gadget "wake; key space"`); a gadget set up before 0.1.9 needs `sudo ihc gadget up --replace` first, to declare remote wakeup |
