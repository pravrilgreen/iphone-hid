...
Chi tiết linh kiện, cách nối, cài đặt và xử lý sự cố: [gadget.md](gadget.md).
...
[phase0-checklist.md](phase0-checklist.md) trả lời câu hỏi này.
...
| USB-serial (CH340), chỉ với cáp CH9329 | Không thấy được từ host. Bài D1 đo bằng `tools/hid_loopback.py` (khoảng cách thực giữa các báo cáo khi tới host). Gadget không có chặng này: báo cáo được ghi thẳng vào `/dev/hidgN`. |
...
`tools/hid_loopback.py` để đọc lại chính xác những gì một host Linux nhận được (chip có mất hay gộp báo cáo không).
...
- **HDMI IN của board, thay cho capture card:** bớt một linh kiện và một cổng USB. Rủi ro: driver HDMI IN của
  board và độ trễ của pipeline mã hoá, phải đo (bài B3, B7; xem [feasibility.md](feasibility.md)).
