...
định. Cấu hình chính là **hộp all-in-one: một Orange Pi 5 Plus cho mỗi iPhone** ([orange-pi-box.md](../guide/orange-pi-box.md)). Các bài
...
**Phần cứng** (chi tiết và cách nối: [orange-pi-box.md](../guide/orange-pi-box.md)):
...
cài gì thêm; với file đã giải nén (`--extract DIR`), các lệnh bên dưới nằm trong `DIR/bin/`. Có internet và muốn
chạy từ mã nguồn thì:
...
Chạy từ mã nguồn thì các lệnh `ihc`, `ihc-hidtest`, `ihc-capture-check` nằm trong `.venv/bin/`. Lệnh cần
quyền root thì gọi `sudo .venv/bin/ihc gadget up`.

**Tập dượt không cần phần cứng** (trên laptop hoặc board): `ihc-hidtest --fake`, gõ `help`;
`ihc serve --sim 2`, mở `http://localhost:8000`.

**Cài đặt iPhone** theo [iphone-setup.md](../guide/iphone-setup.md).
...
```bash
sudo ihc gadget up                      # profile RA (mặc định)
ihc gadget status                       # "USB state configured" = laptop đã nhận
ihc-hidtest --gadget "info; type hello; move 200 0; move 0 200; abstest; bench 100"
```
...
```bash
sudo ihc gadget up --replace --profile A
ihc gadget status                       # interface: keyboard, consumer, absolute
sudo ihc gadget up --replace            # về RA
```
...
ihc-capture-check list                  # HDMI IN có thêm dòng "HDMI input; the source sends: ..."
VID=/dev/videoN                                      # N lấy từ `list`
ihc-capture-check snapshot --device $VID
ihc-capture-check probe --device $VID --seconds 10
```
...
```bash
ihc-capture-check probe --device $VID --encoder mpp      # JPEG phần cứng (mppjpegenc), nếu image có
ihc-capture-check probe --device $VID --encoder gst      # jpegenc, phần mềm
ihc-capture-check probe --device $VID --encoder ffmpeg   # nếu đã cài ffmpeg
ihc-capture-check probe --device $VID --encoder builtin  # bộ đọc riêng của hộp: không cần cài gì
ihc-capture-check probe --device $VID --fps 60
...
Gadget ở profile RA (`sudo ihc gadget up --replace` nếu cần). Mở khoá iPhone và chọn **Allow** nếu
iOS hỏi về phụ kiện.

**1. Chuột tuyệt đối (câu hỏi quan trọng nhất):**

```bash
ihc gadget status                       # USB state configured
ihc-hidtest --gadget "info; abstest"
```
...
```bash
ihc-hidtest --gadget "move 200 0; move 0 200"
ihc-hidtest --gadget 'type Hello iPhone 123 !@#$%^&*()_+-=[]{};:,./<>?'
```
...
```bash
ihc-hidtest --gadget "capscheck n=10"
```
...
```bash
ihc-hidtest --gadget "trial cmd+space n=20"
```
...
```bash
sudo ihc gadget up --replace --profile K
ihc-hidtest --gadget "trial cmd+space n=20"
sudo ihc gadget up --replace
```
...
**5. Nút Home và App Switcher.** Gán nút trong AssistiveTouch trước ([iphone-setup.md](../guide/iphone-setup.md), bước 2),
rồi mở một app và chạy:

```bash
ihc-hidtest --gadget "home"             # nút phải: phải về màn hình chính
ihc-hidtest --gadget "switcher"         # nút giữa: phải mở App Switcher
```
...
sudo ihc gadget up --replace --profile A
ihc-hidtest --gadget "info; abstest"
ihc-hidtest --gadget "abs 2048 2048 buttons=2; sleep 0.1; abs 2048 2048"   # nút phải qua báo cáo tuyệt đối
sudo ihc gadget up --replace            # về RA
```
...
```bash
sudo ./ihc-box-*-linux-aarch64.run                   # file từ bản release; hoặc từ mã nguồn: sudo sh packaging/install.sh
...
([research/china-market.md](china-market.md) §2.5). Sai số tăng rõ giữa các lần là dấu hiệu gia tốc iOS
...
3. Tuỳ chọn: thử pipeline khác trong server. Server chọn bộ mã hoá đầu tiên có sẵn theo thứ tự `mpp`, `gst`,
   `ffmpeg`. Muốn chạy lệnh riêng thì viết `farm.toml` theo mẫu trong [orange-pi-box.md](../guide/orange-pi-box.md), thay `input = "hdmi"`
...
ihc-hidtest --port $PORT "info; cfg; cfg save docs/test-logs/ch9329-cfg-factory.json; bench 200"
ihc-hid-loopback --port $PORT list
ihc-hid-loopback --port $PORT all
