...
- **Sản phẩm chính: mỗi iPhone một Orange Pi 5 Plus** ([orange-pi-box.md](../guide/orange-pi-box.md)):
...
Hai báo cáo mới (tiếng Anh, có nguồn và nhãn tin cậy): [chuột tuyệt đối trên iPhone](absolute-pointer.md)
và [phần cứng giá rẻ của Trung Quốc](china-market.md). Những điểm làm thay đổi đánh giá bên dưới:
...
| A6 | Mỗi báo cáo có xác nhận; xác nhận nghĩa là lệnh đã tới iOS | **[Contradicted]**: ack của CH9329 chỉ là chip đã nhận khung; xác nhận của gadget là iPhone đã lấy báo cáo ở tầng USB, vẫn chưa phải iOS đã xử lý | Tài liệu CH9329 (xem `docs/dev/ch9329-protocol.md`). Aiden: "a successful `/dev/hidg0` write only proves that the gadget accepted the HID report" ([usb-hid.md](https://github.com/AidenAI-IO/aiden-firmware/blob/main/docs/03-services/usb-hid.md)) | Phải có lệnh idempotent (mục 5). Gadget của hộp chờ tới khi iPhone lấy báo cáo, không dừng ở chỗ ghi được vào `/dev/hidgN`. |
...
- **[Confirmed]** (CH9329) Ở 9600 baud, một khung chuột relative kèm ack mất 18 byte trên dây, tức 18,75 ms. Cộng 3 ms ngưỡng "packet interval" của chip, tổng khoảng 22–24 ms. **Pacer 25 ms gần như không còn biên**: mỗi lần trễ hay retry đều làm lệch nhịp, và nhịp lệch thì gia tốc lệch. Ở 115200 baud chỉ còn khoảng 1,6 ms + 3 ms. Gadget không có chặng serial này.
  Nguồn: tính từ khung trong `docs/dev/ch9329-protocol.md`.
...
- **Hệ quả vận hành:**
  ...
  - Low Power Mode ép Auto-Lock về 30 giây, phải tắt (đã có trong `docs/guide/iphone-setup.md`).
...
- **[Confirmed]** Sipeed NanoKVM-Go (59–89 USD) có chế độ chuột tuyệt đối ("Follow Mouse") được ghi cho iPhone 15/16/17, một dây USB-C, khoảng 60 ms ở 1080p60 ([research/china-market.md](china-market.md) §2.9). Cùng ý tưởng "mỗi máy một bo" như hộp; dùng làm mốc đối chiếu (X2).
...
9. **Cài đặt iPhone** (bổ sung cho `docs/guide/iphone-setup.md`):
...
|---|---|---|---|---|
| **B1** | R1: cổng Type-C làm được device | `ls /sys/class/udc`, `ihc gadget status`; nếu trống thì thử image của Orange Pi | Có đúng một UDC, không bị gadget khác chiếm | 15 phút, không cần iPhone |
...
| **B5** | R2, R4, R7: hiệu chỉnh và độ chính xác | `packaging/install.sh`, Calibrate trên console; vuốt, tap lặp, hai mức Tracking Speed; chế độ tương đối ép buộc 3 lần | `mode: absolute`, sai số tối đa ≤ 2 pt (≤ 4 pt nếu relative); 0 lần kẹt kéo | ½ ngày |
