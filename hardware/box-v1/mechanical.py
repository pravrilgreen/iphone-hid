...
The board size, the mounting holes and the connector positions come from placement.py, the
placement pcb.py lays the board out with, so the openings follow the layout. Connector heights come
from the vendors' drawings and must be checked against them; the RJ45 height (13.5 mm) is the
figure used in README §11.

Coordinates: PCB top view, origin at the top-left corner of the board, x to the right, y down the
page, z up from the top surface of the PCB. Units: mm. The left PCB edge is the front of the box
(the iPhone port), the right edge is the rear (Ethernet, PC, power); "top" and "bottom" name the two
long sides as drawn.
"""
...
PCB_W, PCB_H, PCB_T = PL.W, PL.H, 1.6
...
class Opening:
    ...
    wall: str         # left (front of the box), right (rear), top, bottom, lid
    pos: float        # centre along the wall in PCB coordinates (y for left/right, x for top/bottom)
...
def _y(ref):
    return PL.PLACE[ref][1]


RJ45_Y = _y("J402") - 13.94 + (13.94 + 5.06) / 2    # centre of the HR911105A body (footprint rotated 90°)
USBC = (9.6, 3.8)     # opening for a USB-C receptacle whose front face ends at the outer wall surface
LEDS = [(PL.PLACE[r][0], PL.PLACE[r][1]) for r in ("D501", "D502", "D503")]
OPENINGS = [
    Opening("J201", "left", _y("J201"), 1.6, *USBC, "stadium", "USB-C to the iPhone (HRO TYPE-C-31-M-04)",
            "front of the box; receptacle face within 0.5 mm of the outer wall; height from the HRO drawing"),
    Opening("J402", "right", RJ45_Y, 6.9, 16.4, 14.0, "rect", "RJ45 Ethernet (HanRun HR911105A)",
            "body ~16 x 13.5 mm [Likely], jack LEDs visible through the opening"),
    Opening("J401", "right", _y("J401"), 1.6, *USBC, "stadium", "USB-C to the PC (HRO TYPE-C-31-M-12)", ""),
    Opening("J101", "right", _y("J101"), 1.6, *USBC, "stadium", "USB-C PD power input (HRO TYPE-C-31-M-12)", ""),
    Opening("ANT", "top", 84.0, 9.0, 6.5, 6.5, "dhole", "Wi-Fi antenna, RP-SMA bulkhead (option)",
            "IPEX 1.0 pigtail from the Core1106 ANT1 connector; clear of the RJ45 and the H4 post; omit when an "
            "internal FPC antenna sits behind a non-metal window"),
    Opening("D501-D503", "lid", LEDS[1][0], LEDS[1][1], 2.0, 2.0, "round", "3 light pipes for the status LEDs",
            "at y = " + ", ".join(f"{y:.1f}" for _, y in LEDS) + " mm, next to the front wall; 0603 top-view LEDs"),
    Opening("SW501", "lid", PL.PLACE["SW501"][0], PL.PLACE["SW501"][1], 1.5, 1.5, "round",
            "RECOVERY button pin-hole", "top-actuated tactile switch; press with a pin"),
    Opening("SW502", "lid", PL.PLACE["SW502"][0], PL.PLACE["SW502"][1], 1.5, 1.5, "round",
            "RESET button pin-hole", ""),
]

# Outlines of the main parts on the top view, from the placement: (label, x, y, w, h)
_M, _L, _U = PL.MOD, PL.LTC, PL.MCU
PARTS = [
    ("U401 Core1106 30x30", _M[0] - 15, _M[1] - 15, 30.0, 30.0),
    ("U201 LT7911D", _L[0] - 3.75, _L[1] - 3.75, 7.5, 7.5),
    ("U301 CH32V305", _U[0] - 5, _U[1] - 5, 10.0, 10.0),
    ("L101", PL.PLACE["L101"][0] - 5, PL.PLACE["L101"][1] - 5, 10.0, 10.0),
    ("L102", PL.PLACE["L102"][0] - 5, PL.PLACE["L102"][1] - 5, 10.0, 10.0),
    ("J501 J502 headers", 7.7, 0.8, 28.6, 3.6),
]
CONNECTOR_BODIES = {   # courtyards of the connectors in PCB coordinates: (x, y, w, h)
    "J201": (0.0, _y("J201") - 5.3, 9.46, 10.6),
    "J402": (PCB_W - 22.3, RJ45_Y - 9.5, 22.3, 19.0),
    "J401": (PCB_W - 9.42, _y("J401") - 5.32, 9.42, 10.64),
    "J101": (PCB_W - 9.42, _y("J101") - 5.32, 9.42, 10.64),
}
...
def openings_table() -> str:
    ...
    for o in OPENINGS:
        if o.wall == "lid":
            pcb = f"x = {o.pos:.1f}, y = {o.z:.1f}"
            out = "lid, same x/y + wall offset " f"{CLEAR + WALL:.1f} mm"
            size = f"Ø{o.w:.1f}"
        else:
            axis = "y" if o.wall in ("left", "right") else "x"
            wall_name = {"left": "left (front)", "right": "right (rear)"}.get(o.wall, o.wall)
        ...
        wall = wall_name if o.wall != "lid" else "lid"
        lines.append(f"| {o.ref} | {wall} | {pcb} | {out} | {size} | {o.what} | {o.note} |")
    return "\n".join(lines)
...
def top_view(ox, oy):
    ...
    for o in OPENINGS:
        if o.wall == "lid":
            pts = LEDS if o.ref.startswith("D5") else [(o.pos, o.z)]
            for x, y in pts:
                out.append(circle(px + x * S, py + y * S, o.w / 2 * S + 1, LIDC, "#dbeafe"))
            label = "LEDs" if o.ref.startswith("D5") else o.ref
            lx, ly = pts[-1]
            out.append(text(px + (lx + 3) * S, py + ly * S + 4, label, 10, "start", LIDC))
            continue
        ...
        if o.wall in ("left", "right"):
            ...
            y = oy + (0 if o.wall == "top" else (OUTER_H - WALL)) * S
            x = px + (o.pos - o.w / 2) * S
            out.append(rect(x, y, o.w * S, WALL * S, CUT, CUT, 1))
    out.append(text(px + _M[0] * S, py + (_M[1] + 4) * S, "ANT1 IPEX 1.0 on the module", 10, "middle", IND))
    out.append(text(px + _M[0] * S, py + (_M[1] + 6.5) * S, "(location: Core1106.pdf p.2)", 10, "middle", IND))
    ...
    out.append(text(ox - 4, oy + OUTER_H * S / 2, "front", 11, "end", DIM))
    out.append(text(ox + OUTER_W * S + 4, oy + OUTER_H * S / 2, "rear", 11, "start", DIM))
    return out


def elevation(ox, oy, wall: str):
    length = OUTER_W if wall in ("top", "bottom") else OUTER_H
    title = {"left": "Front wall (PCB left edge)", "right": "Rear wall (PCB right edge)",
             "top": "Top side wall (PCB top edge)", "bottom": "Bottom side wall (PCB bottom edge)"}[wall]
    out = [text(ox, oy - 10, title + ", seen from outside", 13, weight="bold")]
...
def render() -> str:
    ...
    out = [f'<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" '
           ...
           text(20, 54, f"Outer {OUTER_W:.0f} x {OUTER_H:.0f} x {OUTER_Z:.1f} mm, wall/lid {WALL:.0f} mm, "
                        ...
                        "Positions follow placement.py (the board layout).", 12, color=DIM),
           text(20, 72, "Red = wall cut-out, blue = lid hole, grey dashed = main parts on the board.",
                12, color=DIM)]
    out += top_view(80, 125)
    out += elevation(660, 150, "left")
    out += elevation(660, 360, "right")
    out += elevation(80, 640, "top")
    out += elevation(660, 640, "bottom")
    # notes
    notes = [
        "Notes",
        ...
        "2. Connector centre heights are nominal: take them from the HRO TYPE-C-31-M-04 / -M-12 and HanRun",
        "   HR911105A drawings.",
