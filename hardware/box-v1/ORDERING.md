...
- **Product:** a box that drives an iPhone (USB-C, iPhone 15 and later) as a USB HID absolute pointer, keyboard and
  media keys, and captures the iPhone's screen over DisplayPort Alt Mode. One 4-layer PCB of ~90×60 mm: LT7911D
...
- **Revision:** box-v1 schematic of 2026-09-26. **Engineering reference:** [README.md](README.md).
...
| Item | Status |
|---|---|
| Schematic, pin level | Complete for every part with a public datasheet; `check.py --selftest` passes (746 pins, 135 nets) |
| LT7911D pinout, footprint, reference circuit, firmware | **Blocking:** datasheet under NDA; 25 of its pins carry placeholder numbers `?..` [Unknown] (§4) |
| Bill of materials | Every orderable line has a manufacturer and MPN; LCSC codes [Likely] (§9) |
| PCB layout (placement, routing, Gerbers, pick-and-place) | **Not in the package: the layout house does it** (§3) |
| Two custom footprints (LT7911D QFN-64, Core1106 castellated module) | To be made by the layout house (§3) |
| Enclosure | Proposal drawing only ([svg/mechanical.svg](svg/mechanical.svg), §8) |
| Firmware and software | Not in the package (§11) |

The package is ready for a **quotation request** for layout and a prototype run. It is **not** ready for a turnkey
production order: the LT7911D documents must be obtained (§4) and the layout approved first.
...
| `kicad/box-v1.kicad_pro`, `kicad/*.kicad_sch` | KiCad 8 schematic (root + 5 sheets), generated. Symbols are embedded; footprints use KiCad 8 standard library names except the two custom ones in §3 |
...
Only Python 3 (standard library) is needed to regenerate and check the files.

## 3. Work split

**Complete in this package:** circuit (except the LT7911D pin numbers), part selection with MPNs, net classes and
impedance targets (in `kicad/box-v1.kicad_pro`), routing and placement rules (README §5, §11), bring-up plan
(README §9), enclosure proposal.

**To be done by the layout house:**

1. Footprints: `box-v1:LT7911D_QFN-64-1EP_7.5x7.5mm_P0.4mm` from the Lontium package drawing (only after §4), and
   `box-v1:Luckfox_Core1106_Castellated_30x30mm_P1.0mm` from Luckfox's `Core1106-SMT` footprint (30×30 mm, 112 pads,
   1.0 mm pitch, 0.7×1.5 mm pads centred on the module edge). Check the L101/L102 land pattern (KiCad
   `L_Bourns_SRP1038C`) against the ordered Bourns SRP1038A-100M.
2. Replace the LT7911D placeholder pins in `netlist.py` (or ask the buyer to) and re-run `generate.py` and
   `check.py`; the check refuses a placeholder that is not labelled [Unknown].
3. Placement and routing to §7, stack-up and impedance confirmation with the fab (§5), DRC.
...
## 4. Blocking item: the LT7911D documents (under NDA)

The LT7911D (Lontium, QFN-64 7.5×7.5 mm, 0.4 mm pitch) converts the iPhone's USB-C DisplayPort Alt Mode output to
4-lane MIPI CSI-2. Its full datasheet and reference designs are only given under NDA. What is known and what is
missing is in README §4.2.

**Request from Lontium or an authorised distributor:**
...
1. Ask the layout house (question 1 of the RFQ, §13) whether it already works with LT7911D documents under an NDA
   with Lontium or a Lontium distributor; many Shenzhen design houses do. If so, it fills in the pinout and
   footprint as part of the layout job.
2. Otherwise contact a Lontium distributor directly and sign the NDA; pass the documents to the layout house under
   the same terms.
...
4. Layout of the rest of the board (power, CH32V305, Core1106, Ethernet, USB) may start with the LT7911D area
   reserved (~15×15 mm next to J201, README §11), accepting some rework risk.
...
| Layer use | L1 parts + all differential pairs; L2 solid GND; L3 power pours (5V_SYS, 3V3, 1V2) + slow signals; L4 slow signals + GND pour |
| Thickness | 1.6 mm ±10 % |
...
| Vias | 0.20 mm finished drill / 0.45 mm pad for signals (as in the KiCad net classes); 0.30 mm drill thermal vias in the exposed pads of U102/U103 (HSOP) and U201 (EPAD); via-in-pad filled and capped preferred for U201's EPAD, otherwise tented from L4 with a window-pane stencil; no blind or buried vias |
...
| Board size | 90 × 60 mm target, 2 mm corner radius; 4× Ø2.7 mm mounting holes (M2.5) 3.5 mm from each corner |
| Edge features | USB-C receptacles (J101, J201, J401) overhang the board edge so that their faces reach the case wall (§8): no V-score along those edges |
| Panelisation | Vendor's choice, e.g. 2×2 with 5 mm rails and tab routing (mouse bites) away from connector edges; 3 global fiducials on the rails, 2 local fiducials near U201 and U301, 1 near U401 |
...
| Polarity / orientation | Diodes: pin 1 = cathode (D101, D201, D401 TVS cathode to the rail; D102 zener cathode to PSW_S; D104 cathode to 5V_SYS). LEDs D501–D503: cathode to GND. C101 electrolytic: + to VIN. Pin 1 of U101–U106, U201–U205, U301, U401, Q101–Q103 and the 4-pad crystals X201/X301 per the silkscreen |
...
- **Placement:** J201 (iPhone USB-C) on the left edge, U202–U205 right behind it, then the LT7911D (≤ 15 mm from
  J201) and the CH32V305 (≤ 30 mm). Core1106 in the centre, edge with pads 1–28 facing the LT7911D. J402 RJ45 on the
  right edge next to module pads 85–88. J401 (PC USB-C) and J101 (PD USB-C) on the bottom edge; the power block
  (U101–U106, Q101–Q103) in the bottom-right corner, away from the DP/CSI pairs. Headers, buttons and LEDs along the
  top edge.
...
board. Outer case about 95 × 65 × 24 mm (2 mm walls, PCB on 3 mm standoffs, RJ45 ~13.5 mm tall).
...
|---|---|---|---|---|---|---|
| J201 | left | y = 20.0, z = +1.6 over PCB top | 22.5 mm, 8.2 mm | 9.6 x 3.8 slot, R1.9 ends | USB-C to the iPhone (Molex 105450-0101) | receptacle face within 0.5 mm of the outer wall; centre height from the Molex drawing |
| J402 | right | y = 16.0, z = +6.9 over PCB top | 46.5 mm, 13.5 mm | 16.4 x 14.0 | RJ45 Ethernet (HanRun HR911105A) | body ~16 x 13.5 mm [Likely], jack LEDs visible through the opening |
| J401 | front | x = 38.0, z = +1.6 over PCB top | 40.5 mm, 8.2 mm | 9.6 x 3.8 slot, R1.9 ends | USB-C to the PC (HRO TYPE-C-31-M-12) | near Core1106 pads 22-23 (README §11) |
| J101 | front | x = 74.0, z = +1.6 over PCB top | 76.5 mm, 8.2 mm | 9.6 x 3.8 slot, R1.9 ends | USB-C PD power input (HRO TYPE-C-31-M-12) | power corner, away from the DP/CSI pairs |
| ANT | back | x = 78.0, z = +9.0 over PCB top | 14.5 mm, 15.6 mm | Ø6.5 D-hole (per bulkhead) | Wi-Fi antenna, RP-SMA bulkhead (option) | IPEX 1.0 pigtail from the Core1106 ANT1 connector; omit when an internal FPC antenna sits behind a non-metal window |
| D501-D503 | lid | x = 42, 46, 50, y = 5.0 | lid, same x/y + wall offset 2.5 mm | Ø2.0 | 3 light pipes for the status LEDs | at x = 42, 46, 50 mm; 0603 top-view LEDs under the lid |
| SW501 | lid | x = 60.0, y = 5.0 | lid, same x/y + wall offset 2.5 mm | Ø1.5 | RECOVERY button pin-hole | top-actuated tactile switch; press with a pin |
| SW502 | lid | x = 66.0, y = 5.0 | lid, same x/y + wall offset 2.5 mm | Ø1.5 | RESET button pin-hole |  |
...
| LT7911D firmware (CSI output, PD roles, EDID) | From Lontium under NDA (§4) | Buyer with Lontium/distributor |
...
|---|---|---|---|
| Components on `bom.csv`, fitted parts (171 pieces) | 42.20 | 42.20 | unit prices in `bom.csv`: $41.33 from listings, $0.87 allowances |
...
| JLCPCB extended-part feeder fee, $3 x 31 lines, spread over the batch | 9.30 | 1.86 | upper bound: every line outside the Basic/Preferred snapshot counted |
...
| **Total without assembly labour** | **56-64** | **49-57** | estimate |
<!-- END GENERATED: cost -->

Not included: layout (non-recurring), assembly labour and setup, stencil, THT soldering, X-ray, shipping, duties,
...
We would like a quotation for the layout, fabrication and assembly of a 4-layer board (about 90 x 60 mm) that
...
Attached: the schematic package (KiCad 8 project, netlist, BOM with MPNs, schematic PDFs/SVGs), ORDERING.md with
...
1. PCB layout from our schematic: two custom footprints (LT7911D QFN-64 7.5 x 7.5 mm 0.4 mm pitch, Core1106
   30 x 30 mm castellated module), impedance-controlled routing (90 ohm USB, 100 ohm DP/CSI/Ethernet differential),
   outputs: Gerber, drill, IPC-D-356, pick-and-place, assembly drawings, STEP model, KiCad files.
...
Questions:
1. Do you have (or can you obtain under NDA through Lontium or its distributor) the full LT7911D datasheet,
   reference schematic for "Type-C DP Alt Mode sink to 4-lane MIPI CSI-2 with pass-through charging", and CSI
   firmware? If yes, can you complete the LT7911D pinout and footprint as part of the layout job?
...
- Open decisions before layout: Option A (default) or B for iPhone charging (README §3.4), the D101 TVS rating
  (README risk H12), C215 value (README §14), antenna path (RP-SMA or FPC), THT soldering by the vendor or the buyer.
