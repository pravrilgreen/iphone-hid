...
#
#     tools/build_bundle.sh            # -> dist/ihc-box-<version>-linux-aarch64.run
#
...
if [ -n "$(git -C "$ROOT" status --porcelain -- ihc web tools deploy 2>/dev/null)" ]; then
    rev="$rev-dirty"
fi
VERSION="$version+$rev"
...
"$HOST_PY" -m pip install --quiet --disable-pip-version-check --no-compile --target "$box/lib" \
    ...
    -r "$ROOT/deploy/bundle/requirements.txt"
find "$box/lib" -name tests -type d -path "*/numpy/*" -prune -exec rm -rf {} +
rm -rf "$box/lib/bin"

echo "== app"
mkdir -p "$box/app/tools" "$box/app/docs" "$box/bin"
cp -r "$ROOT/ihc" "$ROOT/web" "$ROOT/deploy" "$box/app/"
rm -rf "$box/app/deploy/bundle"
cp "$ROOT/tools/hidtest.py" "$ROOT/tools/capture_check.py" "$ROOT/tools/gadget.py" "$box/app/tools/"
cp "$ROOT/docs/gadget.md" "$box/app/docs/"
find "$box/app" -name __pycache__ -type d -prune -exec rm -rf {} +
echo "$VERSION" > "$box/VERSION"
cp "$ROOT/deploy/bundle/install.sh" "$box/install.sh"
cp "$ROOT/deploy/bundle/launcher.sh" "$box/bin/ihc"
...
sed "s/@VERSION@/$VERSION/" "$ROOT/deploy/bundle/run-header.sh" > "$run.part"
