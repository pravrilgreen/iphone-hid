#!/usr/bin/env python3
"""gadget: make this board the phone's USB keyboard and mouse (Linux USB gadget, no HID chip).

    sudo python3 tools/gadget.py up              # keyboard + media keys + relative and absolute mouse
    sudo python3 tools/gadget.py up --profile A  # absolute pointer only (no relative mouse)
    python3 tools/gadget.py status               # is a USB device controller there, did the phone enumerate?
...
from ihc.hid import gadget as g  # noqa: E402
...
def _load_framework(configfs: str) -> None:
    ...
    if not cfg_root.is_dir() or not any(cfg_root.iterdir()):
...
def _print_status(st: dict) -> None:
    ...
    if not st["exists"]:
        print(f"gadget {st['name']!r}: not set up (sudo python3 tools/gadget.py up)")
        return
