"""`ihc gadget`: make this board the phone's USB keyboard and mouse.
...
def _print_status(st: dict) -> None:
    ...
    if not st["exists"]:
        print(f"gadget {st['name']!r}: not set up (sudo ihc gadget up)")
        return
...
def run(args: argparse.Namespace) -> int:
    ...
    print("next: plug the USB-C port into a USB-A port of the phone's hub, unlock the phone, then "
          "`ihc-hidtest --gadget \"info; move 100 0; click\"`")
    return 0
