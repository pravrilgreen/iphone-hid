...
does not move or type.
    ...
    ihc-hid-loopback --port /dev/ttyUSB0 list           # the chip's input devices
    ihc-hid-loopback --port /dev/ttyUSB0 rel            # relative reports at several paces
    ihc-hid-loopback --port /dev/ttyUSB0 abs            # absolute reports (grid)
    ihc-hid-loopback --port /dev/ttyUSB0 keys           # typing
    ihc-hid-loopback --port /dev/ttyUSB0 all
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--log", help="JSON-lines log (default <log dir>/loopback-<time>.jsonl)")
