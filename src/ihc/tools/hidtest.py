...
"""hidtest: interactive checker for a CH9329 (or a Linux USB gadget) during the phase 0 hardware tests.
    ...
    ihc-hidtest --port /dev/ttyUSB0             # shell; type `help`
    ihc-hidtest --port /dev/ttyUSB0 info        # run one command and exit
    ihc-hidtest --port /dev/ttyUSB0 "move 200 0; click"
    ihc-hidtest --gadget                        # this board is the keyboard/mouse (ihc gadget up)
    ihc-hidtest --fake                          # dry run against the simulated chip

Each session appends a JSON-lines log (default <log dir>/hidtest-<time>.jsonl; the log dir is
$IHC_TEST_LOG_DIR, docs/test-logs in a source checkout, else ~/ihc-test-logs) holding every
...
from ihc.tools import log_dir
from ihc.hid import protocol as p
from ihc.hid.base import HidError, HidTimeout
from ihc.hid.ch9329 import CH9329Backend
from ihc.hid.config import ChipConfig, parse_value
from ihc.hid.gadget import GadgetBackend, gadget_status
from ihc.hid.scan import candidate_ports, list_serial_ports
from ihc.input import keymap
from ihc.jsonlog import EventLog
...
class HidShell(cmd.Cmd):
    ...
    def do_cfg(self, arg: str) -> None:
        ...
        cfg save [FILE]          back it up to JSON (default <log dir>/ch9329-cfg-<time>.json)
    ...
    def _cfg_write(self, cur: ChipConfig, new: ChipConfig) -> None:
        ...
        print(f"undo: ihc-hidtest --port {self.hid.port} [--baud N] cfg restore {backup}")
    ...
    def do_home(self, arg: str) -> None:
        """home: right click, which AssistiveTouch must map to Home (docs/guide/iphone-setup.md)."""
    ...
    def do_cmdspace(self, arg: str) -> None:
        """cmdspace [n=20] [...]: shorthand for `trial cmd+space ...` (docs/research/phase0-checklist.md)."""
...
def main(argv: list[str] | None = None, *, ask=input) -> int:
    ...
    ap.add_argument("--log", help="JSON-lines log path (default <log dir>/hidtest-<time>.jsonl)")
