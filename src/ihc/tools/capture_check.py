...
"""capture_check: check a video input (formats, real frame rate, JPEG passthrough, bandwidth).
    ...
    ihc-capture-check probe --device /dev/video0 [--fourcc MJPG] [--size 1920x1080] [--fps 30] [--seconds 10]
    ihc-capture-check multi --device /dev/video0 --device /dev/video2 [--seconds 20]   # several cards at once
    ihc-capture-check snapshot --device /dev/video0 [--out snap.jpg]
...
No image analysis: it measures delivery only. Results go to a JSON-lines log in the log directory (see ihc.tools.log_dir).
"""
...
from ihc.registry import open_video_source
from ihc.video.capture import V4L2Capture, list_video_devices
from ihc.video.diagnose import diagnose
from ihc.video.pipe import is_hdmi_input
from ihc.video.v4l2 import describe_signal
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--out", help="snapshot file (default <log dir>/snap-<time>.jpg)")
