...
from .stream import BOUNDARY, Hubs, MJPEGResponse, Pacer, TooManyViewers, stop_tasks
...
async def _run_until_first_exits(*coros) -> None:
    ...
    try:
        ...
        errors = await stop_tasks(tasks)
    for e in errors:
        if isinstance(e, Exception) and not isinstance(e, (WebSocketDisconnect, RuntimeError, OSError)):
            raise e
