...
class MJPEGResponse(Response):
    ...
    async def _serve(self, w: Watcher, scope, receive, send) -> None:
        ...
        try:
            ...
            await stop_tasks([streamer, listener])
...
async def stop_tasks(tasks: list[asyncio.Future]) -> list[BaseException | None]:
    """Cancel the tasks and wait until they are done; return what each raised (None if nothing).

    Cancelled while it waits, it cancels them again and still waits, then raises its own CancelledError.
    The cancel scopes that cancel a handler (anyio's: the test client, task groups) repeat the cancel on
    every turn of the loop and swallow only their own CancelledError: `asyncio.gather` raised a task's
    instead, which escaped the scope."""
    for t in tasks:
        t.cancel()
    cancelled = None
    # check before each wait: the repeated cancel would otherwise beat the wait's own wake-up every time
    while not all(t.done() for t in tasks):
        try:
            ...
            for t in tasks:
                t.cancel()
    errors = [None if t.cancelled() else t.exception() for t in tasks]  # retrieved: no "never retrieved" logs
