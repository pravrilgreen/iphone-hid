...
def udc_wakeup(udc: str, sysfs: str = "/sys") -> None:
    ...
    try:
        ...
        hint = " (needs root, or the udev rule of packaging/99-ihc.rules)" if e.errno in (errno.EACCES, errno.EPERM) else ""
    ...
    if udc is None:
        if not udcs:
            raise GadgetError("no USB device controller on this board: the USB-C port is in host mode "
                              "(enable its peripheral/OTG mode in the device tree, see docs/guide/orange-pi-box.md)")
        if len(udcs) > 1:
